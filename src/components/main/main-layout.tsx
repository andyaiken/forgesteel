...
export const MainLayout = ({
	...
	const getHeroAbilityModal = useCallback((hero: Hero, segments: string[]) => {
		const [ abilityId ] = segments;
		const ability = HeroLogic.getAbilities(hero, true, true, true)
			.find(a => a.id === abilityId)!;
		return <AbilityModal ability={ability} hero={hero} updateHero={onHeroChange} />;
	}, [ onHeroChange ]);
	...
	const getHeroModal = useCallback(
		...
		[ heroes, sourcebooks, navigation, getHeroAbilityModal, onHeroChange ]
	);
